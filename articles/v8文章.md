...
```python
# v8_team_agent.py: TeammateManager._write_to_inbox()
...
```python
# v8_team_agent.py: TeammateManager._update_team_config()
...
```python
# v8_team_agent.py: Teammate.__post_init__()
def __post_init__(self):
    if not self.agent_id:
        self.agent_id = f"{self.name}@{self.team_name}"
        # 例如: "backend@rest-to-graphql"

# v8_team_agent.py: TeammateManager.spawn_teammate()
